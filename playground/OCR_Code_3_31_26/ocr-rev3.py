...
if display_contour is not None:
    ...
    # Define ROI for OCR (adjust based on display)
    h, w = thresh.shape
    roi = thresh[int(h * 0.16):int(h * 0.48), int(w * 0.28):int(w * 0.63)]

    # OCR
    # Invert ROI if text is white on black
    roi_inv = cv2.bitwise_not(roi)

    # Fill small holes inside digits
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3,3))
    roi_filled = cv2.morphologyEx(roi_inv, cv2.MORPH_CLOSE, kernel)

    # Resize for OCR
    roi_resized = cv2.resize(roi_filled, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)

    # OCR config: digits only
    config_tess = "--psm 8 -c tessedit_char_whitelist=0123456789"
    text = pytesseract.image_to_string(roi_resized, config=config_tess)
