...
if display_contour is not None:
    ...
    warped_gray = cv2.resize(warped_gray, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
    ...
    h, w = thresh.shape
    roi = thresh[int(h * 0):int(h * 0.14), int(w * 0.5):int(w * 0.72)]

    # OCR
    # Invert so letters are black on white
