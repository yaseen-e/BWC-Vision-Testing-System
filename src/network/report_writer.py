...
REPORT_FIELD_NAMES: tuple[str, ...] = ("Mode", "Mode_Conf", "Temp", "Temp_Conf")
...
def open_test_report() -> tuple[object, csv.DictWriter, Path]:
    """Create a timestamped CSV report for this run and write the home-screen header row."""
    ...
    writer = csv.DictWriter(report_file, fieldnames=list(REPORT_FIELD_NAMES))
    writer.writeheader()
    report_file.flush()
